<template>
  <Layout>
    <Breadcrumb title="Animation" subtitle="Extended" />
    ...
    <div class="grid grid-cols-1 gap-6">
      <div class="card">
        <div class="p-6">
          ...
          <p class="sub-header">Just-add-water CSS animations.</p>
          ...
          <div class="flex w-1/2 mx-auto justify-center items-center gap-2 mt-4">
            ...
            <button
              class="grow w-44 rounded-md bg-indigo-600 border border-transparent py-2 px-3 text-sm font-semibold leading-5 text-white hover:bg-indigo-500 js--triggerAnimation"
              type="button"
            >
              Animate Me !
            </button>
...
export default {
  ...
  mounted() {
    const trigger = document.querySelector<HTMLElement>('.js--triggerAnimation')
    const animations = document.querySelector<HTMLElement>('.js--animations')
  ...
  methods: {
    testAnim(x) {
      const sandbox = document.querySelector<HTMLElement>('#animationSandbox')
      if (sandbox) {
