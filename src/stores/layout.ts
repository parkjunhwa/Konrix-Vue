...
export const useLayoutStore = defineStore('layout', {
  ...
  actions: {
    init() {
      ...
      config.layout.width = html.getAttribute('data-layout-width') || this.config.layout.width
      config.layout.position =
        html.getAttribute('data-layout-position') || this.config.layout.position
      config.topbar.color = html.getAttribute('data-topbar-color') || this.config.topbar.color
      config.menu.color = html.getAttribute('data-menu-color') || this.config.menu.color
      config.sidenav.view = html.getAttribute('data-sidenav-view') || this.config.sidenav.view
    ...
    initSidenav() {
      ...
      const menuLinks = document.querySelectorAll<HTMLAnchorElement>('ul.menu a.menu-link')
      ...
      menuLinks.forEach((element) => {
        ...
        if (element.href === pageUrl) {
          ...
          if (parentMenu && parentMenu.classList.contains('menu-item')) {
            const collapseButtonElement = parentMenu.querySelector('.hs-collapse-toggle')
            const collapseElement = parentMenu.querySelector('.hs-collapse')
      ...
      setTimeout(function () {
        const activatedItem = document.querySelector<HTMLElement>('ul.menu .active')
        if (activatedItem != null) {
          const simplebarContent = document.querySelector<HTMLElement>(
    ...
    initSwitchListener() {
      ...
      document.querySelectorAll<HTMLInputElement>('input[name=dir]').forEach(function (element) {
      ...
      if (menuToggleBtn) {
        menuToggleBtn.addEventListener('click', function () {
          const configView = self.config.sidenav.view
      ...
      if (menuHoverToggleBtn) {
        menuHoverToggleBtn.addEventListener('click', function () {
          ...
          if (configView == 'hover') {
            if (view === 'hover') {
              self.changeSidenavView(configView == 'hover' ? 'hover' : configView, true)
            } else {
    ...
    setSwitchFromConfig() {
      document
        .querySelectorAll<HTMLInputElement>('#theme-customization input[type=radio]')
        .forEach(function (radio) {
          radio.checked = false
        })
      const config = this.config
      if (config) {
        const layoutDirectionSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=dir][value=' + config.direction + ']',
        )
        const layoutModeSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=data-mode][value=' + config.theme + ']',
        )
        const layoutWidthSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=data-layout-width][value=' + config.layout.width + ']',
        )
        const layoutPositionSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=data-layout-position][value=' + config.layout.position + ']',
        )
        const topbarColorSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=data-topbar-color][value=' + config.topbar.color + ']',
        )
        const menuColorSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=data-menu-color][value=' + config.menu.color + ']',
        )
        const sidenavViewSwitch = document.querySelector<HTMLInputElement>(
          'input[type=radio][name=data-sidenav-view][value=' + config.sidenav.view + ']',
        )
