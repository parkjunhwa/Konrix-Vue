<template>
  ...
  <button
    ...
    class="fixed hidden h-10 w-10 items-center justify-center rounded-full z-10 bottom-4 end-4 p-2.5 bg-primary cursor-pointer shadow-lg text-white"
  >
  ...
  <div>
    ...
    <div
      ...
      class="fc-offcanvas-open:translate-x-0 translate-x-full rtl:-translate-x-full fixed inset-y-0 end-0 transition-all duration-300 transform max-w-sm w-full z-50 bg-white border-s border-gray-900/10 dark:bg-gray-800 dark:border-white/10"
      tabindex="-1"
    >
      <div
      ...
      >
        ...
        <button type="button" data-fc-dismiss @click="handleCustomizer">
                ...
                <label class="form-label rounded-md" for="menu-color-gradient">
                  ...
                  >
                    <span
                      ...
                      style="background: linear-gradient(135deg, #6379c3 0%, #546ee5 60%)"
                    ></span>
                  </span>
                </label>
                <div class="mt-1 text-md font-medium text-center text-gray-600 dark:text-gray-300">
...
<script lang="ts">
import { useLayoutStore, defaultLayout } from '@/stores/layout'
export default {
  data() {
    return {
  ...
  methods: {
    handleCustomizer() {
      if (this.useLayout.config.direction == 'ltr') {
        document.getElementById('theme-customization')?.classList.remove('rtl:-translate-x-full')
        if (
          document.getElementById('theme-customization')?.classList.contains('translate-x-full')
        ) {
          document.getElementById('theme-customization')?.classList.remove('translate-x-full')
          this.useLayout.showBackdrop()
          return
        }
        document.getElementById('theme-customization')?.classList.add('translate-x-full')
        this.useLayout.hideBackdrop()
        return
      }

      if (this.useLayout.config.direction == 'rtl') {
        document.getElementById('theme-customization')?.classList.remove('translate-x-full')
        if (
          document
            .getElementById('theme-customization')
            ?.classList.contains('rtl:-translate-x-full')
        ) {
          document.getElementById('theme-customization')?.classList.remove('rtl:-translate-x-full')
          this.useLayout.showBackdrop()
          return
        }
        document.getElementById('theme-customization')?.classList.add('rtl:-translate-x-full')
        this.useLayout.hideBackdrop()
        return
      }
    },
